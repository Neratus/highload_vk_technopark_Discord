...
Применение Kafka для асинхронной записи в сервисы с высокой нагрузкой:
...
**Резервирование данных в СУБД**  - [Ссылка](#резервное-копирование)

**Репликация данных в СУБД**  - [Ссылка](#репликации)


### Graceful Shutdown и политика Failover

Система мониторинга здоровья реализует многоуровневые проверки состояния всех компонентов. Liveness probes каждые 10 секунд контролируют активность сервисов, readiness probes проверяют готовность принимать трафик, а startup probes обеспечивают контроль завершения инициализации медленных сервисов. При обнаружении сбоев автоматически выполняются процедуры исключения из балансировщика после 3 последовательных ошибок с последующим восстановлением после 2 успешных проверок. 
...
## Ключевые допущения по производительности ([источник_22])

| Тип нагрузки     | RPS на ядро     | Память на ядро | Обоснование |
|------------------|------------------:|---------------:|------------|
| Лёгкие API       | 2,000 RPS        | 2–4 GB         | Микросервисы на Go, простые запросы |
| Средняя логика   | 1,000 RPS        | 4–8 GB         | Бизнес-логика средней сложности |
| Тяжёлая логика   | 500 RPS          | 8–16 GB        | Сложные вычисления, обработка медиа |
| SSL Termination  | 500 CPS          | 4–8 GB         | SSL handshake (NGINX benchmarks) |
| Голосовые потоки | 200 потоков/сервер | 6 GB/ядро    | Аудио обработка в реальном времени |
| Базы данных      | 500 RPS/ядро     | 12–16 GB       | Тяжёлые запросы, высокая консистентность |

---

## Stateless сервисы в Kubernetes

| Сервис         | Пик. нагрузка | Тип нагрузки   | Replicas | CPU/r | CPU/l | RAM/r | RAM/l | Всего CPU | Всего RAM |
| -------------- | -------------:| -------------- | --------:| -----:| -----:| -----:| -----:| ---------:| ---------:|
| API Gateway    | 140,000 RPS   | SSL Termination | 14      | 20    | 30    | 32 GB | 48 GB | 280       | 448 GB    |
| Auth Service   | 1,842 RPS     | Средний         | 3       | 4     | 6     | 8 GB  | 12 GB | 12        | 24 GB     |
| Chat Service   | 28,038 RPS    | Тяжёлый         | 12      | 24    | 32    | 32 GB | 48 GB | 288       | 384 GB    |
| Media Service  | 1,476 RPS     | Тяжёлый         | 4       | 8     | 12    | 16 GB | 24 GB | 32        | 64 GB     |
| Friend Service | 6,000 RPS     | Средний         | 6       | 8     | 12    | 16 GB | 24 GB | 48        | 96 GB     |
| Profile Service| 921 RPS       | Средний         | 3       | 4     | 6     | 8 GB  | 12 GB | 12        | 24 GB     |
| Search Service | 5,000 RPS     | Тяжёлый         | 4       | 12    | 16    | 32 GB | 48 GB | 48        | 128 GB    |
| Notify Service | 921 RPS       | Лёгкий          | 3       | 4     | 6     | 8 GB  | 12 GB | 12        | 24 GB     |
| Invite Service | 3,681 RPS     | Средний         | 4       | 6     | 8     | 8 GB  | 12 GB | 24        | 32 GB     |
...
## Stateful сервисы (Базы данных)

| Сервис         | Нагрузка       | Тип БД                          | Нод | CPU/нода | RAM/нода | Всего CPU | Всего RAM |
| -------------- | --------------:| -------------------------------- | ---:| -------:| -------:| ---------:| ---------:|
| Cassandra      | 120k RPS       | NoSQL, высокая запись            | 45  | 32      | 384 GB  | 1,440     | 17,280 GB |
| PostgreSQL     | 10k RPS        | Реляционная, ACID                | 6   | 16      | 64 GB   | 96        | 384 GB    |
| Tarantool      | 140k RPS       | In-memory, кэш                   | 6   | 32      | 512 GB  | 192       | 3,072 GB  |
| Neo4j          | 5k RPS         | Графовая                         | 3   | 12      | 48 GB   | 36        | 144 GB    |
| Elasticsearch  | 5k RPS         | Поисковая                        | 5   | 16      | 64 GB   | 80        | 320 GB    |
...
| Сервис            | Нагрузка         | Нод | CPU/нода | RAM/нода | Всего CPU | Всего RAM |
| ----------------- | ---------------: | ---:| --------:| --------:| ---------:| ---------:|
| L4 Балансировщик  | 533 Гбит/с       | 7   | 24       | 128 GB   | 168       | 896 GB    |
| L7 Балансировщик  | 140k RPS         | 4   | 24       | 128 GB   | 96        | 512 GB    |
| Voice SFU         | 630 потоков      | 4   | 32       | 192 GB   | 128       | 768 GB    |
| Kafka Brokers     | 100k msg/sec     | 9   | 64       | 256 GB   | 576       | 2,304 GB  |
| Ceph Storage      | ~1.06 ПБ/день    | 12  | 24       | 128 GB   | 288       | 1,536 GB  |
| Kubernetes Nodes  | -                | 60  | 64       | 256 GB   | 3,840     | 15,360 GB |

**Итого (infrastructure):** Нод = **96**, 
Всего CPU = **5,096**, 
Всего RAM = **21,376 GB**

> Примечание: для Ceph указана скорость/объём (1.06 ПБ/день) — это объём генерируемых медиа, не RPS.

---

## Сводка по ресурсам и хостингу

**Итоговые требования конфигурации:**

Stateful: 32 CPU, 384–512 GB RAM

Stateless: 32 CPU, 32–64 GB RAM

Инфраструктура: 64 CPU, 128–256 GB RAM

### Рекомендуемые модели серверов
([источник_23]) ([источник_24])

| Категория      | Сервер                         | Характеристики                                   | Аренда/мес (USD)        | Покупка (USD)        |
|----------------|---------------------------------|--------------------------------------------------|--------------------------|------------------------|
| Stateless      | CyberServe EPYC 9004 1U         | AMD EPYC (до 32 ядер), до 256 GB RAM, 1U         | $347.31 – $405.19        | $5,877 – $8,234        |
| Stateful       | CyberServe EPYC 9004 2U         | 32–64 ядер, до 1 TB RAM, 8–12 NVMe               | $578.85 – $752.50        | $7,003 – $12,455       |
| Инфраструктура | CyberServe EPYC 9004 High-Density | 64–96 ядер, до 2 TB RAM, ускоренные NIC         | $810.39 – $1,099.82      | $10,123 – $16,831      |


### Сводка по ресурсам и хостингу

| Категория         | Серверов | Всего CPU | Всего RAM     | Покупка (USD) | Аренда/мес (USD) | Точка окупаемости |
|-------------------|----------|-----------|----------------|----------------|-------------------|--------------------|
| Stateless (K8s)   | 60       | 3,840     | 15,360 GB      | ~ $420,000     | ~ $22,500         | ≈ 19 месяцев       |
| Stateful (DB)     | 65       | 1,844     | 21,200 GB      | ~ $635,000     | ~ $43,250         | ≈ 15 месяцев       |
| Инфраструктура    | 96       | 5,096     | 21,376 GB      | ~ $1,295,000   | ~ $91,700         | ≈ 14 месяцев       |
| **Всего**         | 238      | 10,916    | 58,480 GB      | ~ $2.35 млн    | ~ $157,450        | ≈ 15 месяцев       |


### Ключевые метрики проектирования

- **Общая производительность:** 140,000 RPS пиковой нагрузки  
- **Запас мощности:** 20–30% для stateless, 100% для критичных stateful сервисов  

- **Геораспределение:** 11 региона с автономной работой


### Примечания по расчётам

- **Коэффициенты резервирования:**  
  - Stateless сервисы: **1.3×** (30% запас)  
  - Stateful сервисы: **2.0×** (100% запас для отказоустойчивости)  
  - Инфраструктура: **1.5×** (50% запас для пиковых нагрузок)
...
22) https://www.techempower.com/benchmarks/
23) https://www.broadberry.eu/amd-epyc-9004-rackmount-servers#:~:text=2.5%22%20Drives%20NVMe%20Drives,Configure
24) https://www.hetzner.com/dedicated-rootserver
...
[источник_22]: https://www.techempower.com/benchmarks/

[источник_23]: https://www.broadberry.eu/amd-epyc-9004-rackmount-servers#:~:text=2.5%22%20Drives%20NVMe%20Drives,Configure

[источник_24]: https://www.hetzner.com/dedicated-rootserver