...
| Таблица             | Назначение                                                                 | Основные поля                                                                      | Ключевые связи                                                            |
| :------------------ | :------------------------------------------------------------------------- | :--------------------------------------------------------------------------------- | :------------------------------------------------------------------------ |
| **users**           | Основная информация о пользователе (аккаунт, аутентификация)               | `user_id`, `login`, `email`, `password`, `status`, `created_at`                    | `1 → N` с `sessions`, `notifications`, `profiles`, `blacklist`            |
| **sessions**        | Активные сессии пользователя (токены авторизации, истечение)               | `session_id`, `user_id`, `token`, `expires_at`                                     | `N → 1` с `users`                                                         |
| **profiles**        | Отображаемый профиль пользователя (ник, тег, аватар, описание)             | `profile_id`, `user_id`, `nickname`, `tag`, `description`, `is_male`, `birthday`   | `N → 1` с `users`; `1 → N` с `messages`, `channel_members`, `invitations` |
| **channels**        | Каналы общения — личные чаты, групповые, голосовые                         | `channel_id`, `chat_name`, `chat_desc`, `created_at`                               | `1 → N` с `messages`, `channel_members`, `invitations`                    |
| **messages**        | Сообщения, отправленные профилями в чатах                                  | `message_id`, `channel_id`, `sender_profile_id`, `content`, `status`, `created_at` | `N → 1` с `profiles` и `channels`                                         |
| **channel_members** | Связующая таблица между профилями и чатами                                 | `channel_id`, `profile_id`, `role`, `joined_at`                                    | `M → N` между `profiles` и `channels`                                     |
| **invitations**     | Приглашения в каналы (по ссылке, коду и т.д.)                              | `invite_id`, `channel_id`, `created_by_profile_id`, `code`, `expires_at`           | `N → 1` с `channels` и `profiles`                                         |
| **notifications**   | Системные уведомления пользователя (добавление, упоминание, ошибка и т.п.) | `notification_id`, `user_id`, `notification_type`, `content`, `read_at`            | `N → 1` с `users`                                                         |
| **blacklist**       | Список блокировок пользователей (мьют, бан, игнор)                         | `block_id`, `user_id`, `blocked_user_id`, `reason`, `created_at`                   | `user_id` и `blocked_user_id` → `users`                                   |
